...
function isString(something) {
  if (typeof something === 'object' && !(something instanceof String))
    return false;
  // eslint-disable-next-line prettier/prettier, no-extra-boolean-cast
  return typeof something === 'object' || typeof something === 'string';
}
...
function isPalindrome(str) {
  // throw new Error('Not implemented');
  return (
    str.toLocaleLowerCase().replace(/[^a-zA-Z]+/g, '') ===
    str
      .toLocaleLowerCase()
      .split('')
      .reverse()
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((letter) => outputArray[inputArray.indexOf(letter)])
    .join('');
}
